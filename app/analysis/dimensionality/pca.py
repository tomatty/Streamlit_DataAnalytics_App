...
**1. 目的の明確化**
- 次元削減: 多数の変数を少数の主成分に集約
- データの可視化: 2次元・3次元での散布図表示
- 多重共線性の回避: 相関の高い変数をまとめる
- ノイズ除去: 分散の小さい成分を除外

**2. データの準備**
- **データ形式**:
  - 行：サンプル/観測（例: 顧客、製品、時点）
  - 列：数値型変数（例: 売上、年齢、スコア）
  - 最低2列以上の数値型変数が必要
  - サンプル数は変数数より多い方が望ましい
- **データ例**:
  ```
  | サンプルID | 変数1 | 変数2 | 変数3 | 変数4 |
  |-----------|-------|-------|-------|-------|
  | 1         | 23.5  | 45.2  | 12.1  | 67.8  |
  | 2         | 34.1  | 52.3  | 15.4  | 72.3  |
  | 3         | 28.9  | 48.7  | 13.2  | 69.5  |
  ```
- 欠損値の処理（削除または補完）
- 外れ値の確認と処理
- **標準化は必須**（単位の異なる変数を比較するため）
- **カテゴリー変数の扱い**:
  - PCAは数値データが前提（共分散・相関を計算するため）
...
**3. 主成分数の決定**
- スクリープロット: 累積寄与率が70-90%になる成分数
- カイザー基準: 固有値1以上の成分を採用
- 解釈可能性: ビジネス的に意味のある成分数

**4. 結果の解釈**
- 寄与率: 各主成分が説明する分散の割合
- 因子負荷量: 各変数と主成分の相関（絶対値0.4以上が有意）
