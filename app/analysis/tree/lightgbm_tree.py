...
def show_lightgbm_tree(df: pd.DataFrame):
    ...
    with st.expander("📖 一般的な分析手順", expanded=False):
        st.markdown(
            """
### LightGBM決定木分析の基本的な流れ

**1. 目的の明確化**
- 高精度な予測モデル構築: 従来の決定木より高い予測精度
- 大規模データの処理: 高速な学習が可能
- 特徴量重要度の把握: どの変数が予測に重要か
- 非線形関係の捉え: 複雑なパターンを自動的に学習

**2. LightGBMとは**
- **勾配ブースティング**: 複数の弱い学習器を組み合わせて強力なモデルを構築
- **Gradient Boosting Decision Tree (GBDT)**: 決定木をベースにしたアンサンブル学習
- **高速性**: ヒストグラムベースのアルゴリズムで大規模データに強い
- **高精度**: Kaggleなどのコンペティションで頻繁に使用される
...
- **データ形式**: 決定木分析と同じ
  ...
  | 2         | 35     | 75000  | 5      | 0       |
  ```
- 数値型変数が基本（カテゴリカル変数は自動処理可能）
- 欠損値の取り扱いが可能（LightGBMの強み）
- サンプル数は多いほど良い（数百件以上推奨）
- **カテゴリー変数の扱い**:
  - **ラベルエンコーディング（推奨）**: カテゴリーを整数に変換
    - 例: 性別（男/女）→ 0/1、地域（東京/大阪/名古屋）→ 0/1/2
    - LightGBMは木の分岐にカテゴリーを直接使用できるため効率的
    - ワンホットエンコーディングは不要（次元が増えて非効率）
  - **categorical_feature引数**: LightGBMのカテゴリー列の自動処理機能
  - 順序の有無は問わない（ツリーベースモデルの特性）

**4. ハイパーパラメータの設定**
- **n_estimators（木の数）**: 50-1000程度
  - 多いほど精度向上だが過学習のリスク
- **max_depth（木の深さ）**: 3-10程度
  - 深いほど複雑なパターンを捉えるが過学習しやすい
- **learning_rate（学習率）**: 0.01-0.3程度
  - 小さいほど慎重に学習（n_estimatorsと合わせて調整）
- **num_leaves（葉の数）**: 2^max_depth以下
  - 多いほど表現力が高いが過学習のリスク
...
**7. 特徴量重要度の分析**
- **Gain**: 各特徴量がモデル全体の性能向上にどれだけ寄与したか
- **Split**: 各特徴量が分割に使われた回数
- 重要な特徴量を特定してビジネス洞察を得る
...
**9. 通常の決定木との違い**
- **高精度**: アンサンブル学習により単一の決定木より高精度
- **過学習への耐性**: 正則化パラメータで調整可能
- **高速**: ヒストグラムベースで大規模データに対応
- **欠損値対応**: 欠損値を自動的に処理
- **解釈性**: やや低下（複数の木の組み合わせのため）
